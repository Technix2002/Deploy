# usage .\Gigaware-USBtoSerial.ps1
...
$appremove = 'Gigaware'
$URL = 'http://www.radioshack.com/graphics/uc/rsk/Support/SoftwareDownload/2600949.exe'
$sezip = '2600949.exe'
$file = 'setup.exe'
$switches = '/s /v""/qn ALLUSERS=1 REBOOT=ReallySuppress""'
$appfolder = "C:\$ProgFiles\Gigaware"
...
$wc.DownloadFile($URL, "$target\$sezip")


# to extract files 
$newProc=([WMICLASS]"root\cimv2:win32_Process").create("""$target\$sezip"" /auto $target")
...
$newProc=([WMICLASS]"root\cimv2:win32_Process").create("C:\Windows\System32\msiexec.exe /i ""$target\VISTA\$file"" $switches")
While(Get-WmiObject Win32_Process -filter "ProcessID='$($newProc.ProcessID)'"){start-sleep -seconds 5}
